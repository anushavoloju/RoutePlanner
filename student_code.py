...
def getEuclidianDistance(pointA, pointB):
...
class PriorityQueue:
    ...
    def addCity(self, cityIndex, f, pathFromStart):
        heapq.heappush(self.cityList, (f, { 'cityIndex': cityIndex, 'f': f, 'pathFromStart': pathFromStart }))
        self.count += 1
    
    def getNextCity(self):
        if self.count > 0:
            self.count -= 1
            return heapq.heappop(self.cityList)[1]
        return None
...
def shortest_path(M,start,goal):
    ...
    for intersection in M.intersections.items():
        vertex = City(intersection[0], intersection[1][0], intersection[1][1])
    ...
    for index, cityRoads in enumerate(M.roads):
        vertices[index].h = getEuclidianDistance((vertices[index].x, vertices[index].y), (vertices[goal].x, vertices[goal].y))
        vertices[index].children = []
        for city in cityRoads:
            vertices[index].children.append({ 'cityIndex': city, 'distance': getEuclidianDistance((vertices[index].x, vertices[index].y), (vertices[city].x, vertices[city].y)) })
    
    shortestDistance = None
    shortestPath = None
    openList.addCity(start, vertices[start].h, [start])
    while (openList.hasNextCity()):
        currentElement = openList.getNextCity()
        visitedElement = closedList.get(currentElement['cityIndex'], None)
        if visitedElement:
            if visitedElement['f'] > currentElement['f']:
                closedList[currentElement['cityIndex']] = { 'f': currentElement['f'], 'pathFromStart': currentElement['pathFromStart'] }
        else:
            if (currentElement['cityIndex'] == goal) and ((not shortestDistance) or (shortestDistance > currentElement['f'])):
                shortestDistance = currentElement['f']
                shortestPath = currentElement['pathFromStart']
            closedList[currentElement['cityIndex']] = { 'f': currentElement['f'], 'pathFromStart': currentElement['pathFromStart'] }
            for childElem in vertices[currentElement['cityIndex']].children:
                childF = currentElement['f'] + childElem['distance']
                childPath = [*currentElement['pathFromStart'], childElem['cityIndex']]
                openList.addCity(childElem['cityIndex'], childF, childPath)
